...
Particle::Particle(RenderTarget& target, int numPoints, Vector2i mouseClickPosition) : m_A(2, numPoints)
{
    ...
    m_vx = (float)(rand() % 2 == 0 ? 1 : -1) * ((float)(rand() % 401) + 100);
    ...
    for (int j = 1; j <= numPoints; j++)
    {
        ...
        theta += dTheta;
        m_A(0, j - 1) = m_centerCoordinate.x + dx;
        m_A(1, j - 1) = m_centerCoordinate.y + dy;
    }
}
...
void Particle::draw(RenderTarget& target, RenderStates states) const
{
    ...
    for (int j = 1; j <= m_numPoints; j++) {
        Vector2f pixelCoord = (Vector2f)target.mapCoordsToPixel(Vector2f(m_A(0, j - 1), m_A(1, j - 1)), m_cartesianPlane);
